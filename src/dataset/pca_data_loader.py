...
MNIST_DATA_DIR = cfg.RAW_DATA_DIR
PCA_DATA_CACHE = os.path.join(cfg.CACHE_DATA_DIR, "pca_data_loader")
...
def load_mnist_data():
    ...
    """
    mnist_train = datasets.MNIST(root=MNIST_DATA_DIR, train=True, download=True)
    mnist_test = datasets.MNIST(root=MNIST_DATA_DIR, train=False, download=True)
...
def pca_data_loader(n_components=10, batch_size=64):
    ...
    # 读取缓存文件加速
    cache_dir = os.path.join(PCA_DATA_CACHE, f"pca-components-{n_components}")
